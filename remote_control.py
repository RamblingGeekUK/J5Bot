...
_default_camera_image = create_default_image(1024, 768)
...
def handle_setFreeplayEnabled():
    ...
    if flask_app.remote_control_vector:
        ...
        if isFreeplayEnabled:
            print("FreePlay : Enabled")
            connection.release_control()
        else:
            print("FreePlay : Disabled")
...
# endpoint to test
@flask_app.route("/say/<string:say>", methods=["GET"])
def talk(say):
    decoded = base64.b64decode(say).decode('utf-8')
    print(decoded)

    print(say)
    with anki_vector.Robot() as robot:        
        robot.behavior.say_text(decoded, True)
    return ""
